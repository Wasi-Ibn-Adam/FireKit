...
abstract class FragmentHandler(
    ...
    private var screenSplit: Boolean = false
    ...
    override fun push(f: AuthFragment, map: HashMap<Int, View>?, delayMillis: Long) {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            try {
                ...
                f.handleSplit = screenSplit
                f.handleOrientation = screenOrientation
    ...
    override fun handleView(split: Boolean, portrait: Boolean) {
        this.screenSplit = split
        this.screenOrientation = portrait
        try {
            val cur = (getCurr())
            cur.handleView(split, portrait)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override fun getFragment(_class: Class<*>): AuthFragment {
        return (manager.fragments.stream()
            .filter { fragment: Fragment? -> fragment?.javaClass == _class }
            .findFirst()
            .orElse(null)) as AuthFragment
    }

    override fun getCurr(): AuthFragment {
        val frag = list[list.size - 1]
        return (manager.fragments.stream()
            .filter { fragment: Fragment? -> fragment?.javaClass?.simpleName == frag.name }
            .findFirst()
            .orElse(null)) as AuthFragment
    }
